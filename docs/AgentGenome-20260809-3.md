...
- **约束不变**:employees 只读知识树;写入必须通过架构员工的知识更新 Procedure(§6.2),防止认知漂移。
...
- 结果契约:Procedure 的 prompt 明确要求"最后将结构化结果写入 `output_dir/result.json`",runtime 校验其存在与 schema,不合格视为 Job 失败并重试一次。
...
| 版本面 | Git(workspace + 子仓) | 代码变更、知识/规则/Procedure 的每次修改(commit/PR/diff/作者) | 资产变成了什么样 |
...
| 知识/规则/Procedure 变更 | 基因组管理页:版本历史、变更 diff、来源任务链接(哪个任务的经验触发了这次更新) |
